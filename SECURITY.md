...
The profile's contribution snake is generated by the repository-owned, zero-dependency `scripts/generate-contribution-snake.mjs`. Pull requests exercise it only against local fixtures; their test job grants only `contents: read`, checkout does not persist credentials, and no token is passed to the test process. Trusted `main`, scheduled, and manual Pages runs query GitHub's GraphQL contribution calendar using the ephemeral `GITHUB_TOKEN` only in the generator step. The token has read-only repository permissions, generated SVGs are treated as build artifacts, and the separate deploy job contains only GitHub-owned actions and is the sole holder of `pages: write` and `id-token: write`.
