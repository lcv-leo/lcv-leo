...
- Dependabot prepares dependency pull requests from this repository's own `.github/dependabot.yml`. Eligible Dependabot pull requests may use GitHub's native auto-merge only after this repository's ruleset and required checks pass on the exact head SHA; no external repository or custom controller operates their lifecycle.
