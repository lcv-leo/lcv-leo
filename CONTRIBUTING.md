...
- Preserve Dependabot checks every day, including weekends, at 05:00 in Brasília (`Etc/GMT+3`), automatic rebasing, and GitHub's post-merge branch deletion. The native cron schedule includes weekends; `daily` would check only weekdays. Official Actions under `actions/*` and `github/*` are evaluated immediately; third-party GitHub Actions observe a seven-day cooldown. Security updates are grouped separately per ecosystem and are not delayed by that cooldown.
